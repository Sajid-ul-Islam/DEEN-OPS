...
try:
        ...
        for _, group in df_out.groupby(order_col, sort=False):
            ...
            for idx in range(len(group)):
                if idx < len(parsed_items):
                    slug = parsed_items[idx].get("outlet_slug") or parsed_items[
                        idx
                    ].get("outlet_name")
                    item_outlets.append(
    ...
    def compute_sip_stats(
        ...
        outlet_counts = (
            df[outlet_col].value_counts().to_dict() if outlet_col in df.columns else {}
        )
        multi_item_orders = 0
        split_orders = []
...
def main():
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="Path to output file (default: <input>_with_outlets.xlsx)",
    )
    parser.add_argument(
        "--order-col",
    ...
    parser.add_argument(
        "--target-col",
        default="Item Outlet",
        help="New column name (default: 'Item Outlet')",
    )
    parser.add_argument(
        "--raw-slugs",
        action="store_true",
        help="Keep raw outlet slugs instead of canonical names",
    )
    ...
    if item_col in processed_df.columns and qty_col in processed_df.columns:
        ...
        if not wh_listing.empty:
            wh_path = (
                input_path.parent / f"{input_path.stem}_warehouse_product_listing.xlsx"
            )
            wh_tot_units = int(wh_listing[qty_col].sum())
            print("\n" + "-" * 60)
            print(
                f"  WAREHOUSE PRODUCT LISTING (PICKING LIST): {wh_tot_units} Total Units across {len(wh_listing)} SKUs"
            )
            print("-" * 60)
            for idx, r in wh_listing.head(10).iterrows():
                sku_str = (
                    f" [{r[sku_col]}]" if sku_col and pd.notna(r.get(sku_col)) else ""
                )
    ...
    try:
        ...
        if not pathao_df.empty:
            ...
            print("\n" + "-" * 60)
            print(
                f"  PATHAO BULK CONSIGNMENTS: {len(pathao_df)} Total Consignments | Total COD: Tk {tot_cod:,}"
            )
            print("-" * 60)
            split_p = pathao_df[
            ...
            if not split_p.empty:
                ...
                for _, r in split_p.iterrows():
                    print(
                        f"    * ID: {r['MerchantOrderId']:<12} | COD: Tk {r['AmountToCollect(*)']:>5} | Outlet: {r['WarehouseOutlet']} ({r['ItemQuantity']} items)"
                    )
