...
def render_return_analytics_tab():
    ...
    with st.spinner("Fetching return data from Google Sheets..."):
        try:
            ...
            st.info("Click the button below to fetch real-time order details from WooCommerce and live tracking statuses from Pathao. This may take a moment for large datasets.")
            ...
            if "Order ID" in df.columns:
                ...
                if st.button("🚀 Fetch External Data & Enrich", use_container_width=True, on_click=clear_enrichment):
                    df_to_match = df.copy()
                ...
                if "enriched_returns" in st.session_state:
                    ...
                    st.success(f"Matched {len(enriched_df)} items with WooCommerce & Pathao!")
